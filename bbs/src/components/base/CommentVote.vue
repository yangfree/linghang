  ...
  <div class="comment">
    <ul class="comment-list">
      <li v-for="(item,index) in commentList" :key="index">
        ...
        <div class="bg">
          <ul>
            <li v-for="(reply,index) in item.list" v-show="index<item.defaultInfo.num" :key="index">
          ...
          <div class="reply-btn" v-show="item.list!==0&&item.list.length>2">
            <a href="javascript: void 0" @click="showMore(item)">{{item.defaultInfo.title}}</a>
          </div>
        </div>
      </li>
    </ul>
...
export default {
  ...
  methods: {
    ...
    replyDoThing(reply) {
      // console.log(reply.pid, reply.create_by, reply.current_pet_name);
      this.$emit(
        "sendUser",
